<template>
    ...
    <Dialog v-if="showModal" v-model:visible="showModal" modal header="Informasi Rekening" :style="{ width: '50vw' }">
        <div style="text-align: center; line-height: 1; font-size: 20px; margin-top: 10px">
            ...
            <div>
                <h3>Jumlah Tarik:</h3>
                <InputText type="text" v-model="jumlahTarik" @input="formatJumlahTarik" class="custom-input" :class="{ 'p-invalid': jumlahTarikError }" required />
            </div>
...
export default {
    ...
    methods: {
        ...
        async prosesTarikDana() {
            ...
            try {
                const response = await axios.post(`http://localhost:8000/api/v1/nasabah/tarik`, {
                    norek: Number(this.norek),
