                ...
                <form @submit.prevent="getAccountInfo">
                    <div>
                        <div>
                            ...
                            <InputText type="text" v-model="nomorRekeningPengirim" class="custom-input" :class="{ 'p-invalid': nomorRekeningError }" required />
    ...
    <Dialog v-if="showAccountInfo" v-model:visible="showAccountInfo" modal header="Informasi Rekening" :style="{ width: '50vw' }">
        <div style="text-align: center; line-height: 1; font-size: 20px; margin-top: 10px">
            ...
            <div>
                ...
                <InputText type="index" v-model="nomorRekeningPenerima" class="custom-input" :class="{ 'p-invalid': nomorRekeningPenerimaError }" required />
                <span v-if="nomorRekeningPenerimaError" class="p-error">Nomor rekening penerima harus diisi</span>
            </div>
            <div>
                &nbsp;
                <h5>Jumlah Transfer:</h5>
                <InputText type="text" v-model="jumlahTransfer" @input="formatJumlahTransfer" class="custom-input" :class="{ 'p-invalid': jumlahTransferError }" required />
            </div>
            &nbsp; &nbsp; &nbsp;
...
export default {
    ...
    methods: {
        ...
        async getAccountInfo() {
            ...
            try {
                const response = await axios.get(`http://localhost:8000/api/v1/nasabah/accountInfo/${this.nomorRekeningPengirim}`);
        ...
        async transfer() {
            ...
            try {
                // Lakukan transfer
                const response = await axios.post(`http://localhost:8000/api/v1/nasabah/transfer`, {
