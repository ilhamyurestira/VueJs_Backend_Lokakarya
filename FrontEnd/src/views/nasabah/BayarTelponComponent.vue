<template>
                ...
                <form @submit.prevent="getAccountInfo">
                    <div>
                        <div>
                            ...
                            <InputText type="text" v-model="nomorRekening" class="custom-input" :class="{ 'p-invalid': nomorRekeningError }" required />
                            <span v-if="nomorRekeningError" class="p-error">Nomor rekening harus diisi</span>
                        </div>
                        <div>
                            &nbsp;&nbsp;
                            <h5>Masukkan Nomor Telpon:</h5>
                            <InputText type="text" v-model="nomorTelpon" class="custom-input" :class="{ 'p-invalid': nomorTelponError }" required />
    ...
    <Dialog v-if="showPaymentModal" v-model:visible="showPaymentModal" modal header="Informasi Rekening" :style="{ width: '50vw' }">
        <div style="text-align: center; line-height: 1; font-size: 20px; margin-top: 10px">
            ...
            <p>Jumlah Tagihan: Rp. {{ numberWithDot(tagihan) }}</p>
...
export default {
    ...
    methods: {
        ...
        async getAccountInfo() {
            ...
            // Memeriksa tagihan
            try {
                const response = await axios.get(`http://localhost:8000/api/v1/nasabah/cek-tagihan`, {
                    params: {
        ...
        async bayarTagihan() {
            ...
            try {
                const response = await axios.post(`http://localhost:8000/api/v1/nasabah/bayar-telpon`, {
                    nomorRekening: this.nomorRekening,
                    noTelp: this.nomorTelpon
                });
